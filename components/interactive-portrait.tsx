...
export default function InteractivePortrait() {
  ...
  useEffect(() => {
    ...
    const bgPlaneMaterial = new THREE.MeshBasicMaterial({ color: 0x0a1628, transparent: true })
    ...
    bgPlaneMaterial.onBeforeCompile = (shader) => {
      ...
      `.replace(
        ...
        vec3 colorSoftShape = vec3(0.98, 0.95, 0.92);
        vec3 colorLine = vec3(1.0, 0.34, 0.13); // PEARS orange
        ...
        vec3 finalColor = mix(baseColor, colorLine, lineMix * 0.3);
  ...
  return (
    <div
      ref={containerRef}
      className="fixed inset-0 w-full h-full bg-[#0a1628] cursor-crosshair overflow-hidden"
      style={{ touchAction: "none" }}
    >
      <div className="absolute bottom-4 left-4 z-10 pointer-events-none bg-[#ff5722] px-4 py-2 rounded-full">
        <span className="text-white font-black text-sm tracking-wider">PEARS</span>
      </div>
    </div>
  )
}
